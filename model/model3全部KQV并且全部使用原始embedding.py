...
class Event_Model(nn.Module):
    def __init__(self, args):
        ...
        self.event_bc_list = nn.ModuleList(
            [nn.Linear(self.hidden_dim, 2) for _ in range(len(self.event_type_list))])
        self.argument_bc_list = nn.ModuleList(
            [nn.Linear(self.embedding_dim, 2) for _ in range(len(all_slots))])

        self.projection = nn.Linear(2 * self.embedding_dim, self.embedding_dim)
        self.event_projection = nn.Linear(self.embedding_dim, self.hidden_dim)
        ...
        self.K3 = nn.Linear(self.embedding_dim, 128)
        self.Q3 = nn.Linear(self.embedding_dim, 128)
        self.V3 = nn.Linear(self.embedding_dim, self.embedding_dim)
    ...
    def forward(self, tokens_id, attention_id, segment_id, entity_id, question, argument_id, event_label=None):
        ...
        if self.args.slot:
            ...
            scaleed_slot_score = slot_score
            slot2seq_score = torch.softmax(scaleed_slot_score, dim=2)
            '''算论元att'''
            ...
            scaleed_seq_score = seq_score
            seq2slot_score = torch.softmax(scaleed_seq_score, dim=2)
            '''论元'''
            seq_attention_slot_embedding = torch.bmm(seq2slot_score, slotType_embedding)
            ...
            for i, bc in enumerate(self.argument_bc_list):
                current_argument_logits = bc(argument_logits)  # batch * maxlen *2
                ...
                current_pred = torch.argmax(current_argument_logits, dim=-1).unsqueeze(
                    -1)  # batch*maxlength*2-->>batch*maxlen *1
                if argument_predict == []:
                    argument_predict = current_pred
                else:
                    argument_predict = torch.cat((argument_predict, current_pred), dim=-1)

            '''event2slot 并且获取schema embedding'''
            slot_attention_seq_embedding = torch.bmm(slot2seq_score, seq_embedding)+slotType_embedding  # batch * slot * embedding_dim
            event_query=self.Q3(eventType_embedding)
            slot_key,slot_value=self.K3(slot_attention_seq_embedding),self.V3(slot_attention_seq_embedding)
            event2slot_score=torch.bmm(event_query,torch.transpose(slot_key,2,1))
            event2slot_score=torch.softmax((event2slot_score),dim=2)
            schema_res = torch.bmm(event2slot_score, slot_attention_seq_embedding)
        ...
        if self.args.attention:
            return total_loss, \
                   loss2, \
                   return_logits.transpose((1, 0)), \
                   argument_predict.detach().cpu().numpy(), \
                   torch.argmax(one_hot_gold_label_1, dim=-1), \
                   torch.argmax(label_one_hot, dim=-1), \
                   slot2seq_score, \
                   seq2slot_score, \
                   event2slot_score, \
                   event2seq_score
