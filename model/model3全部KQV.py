...
class Event_Model(nn.Module):
    ...
    def forward(self, tokens_id, attention_id, segment_id, entity_id, question, argument_id, event_label=None):
        ...
        if self.args.slot:
            ...
            '''论元'''
            seq_attention_slot_embedding = torch.bmm(seq2slot_score, slot_value)
            ...
            slot_attention_seq_embedding = torch.bmm(slot2seq_score, seq_value)  # batch * slot * embedding_dim
            event_query=self.Q3(eventType_embedding)
            slot_key,slot_value=self.K3(slotType_embedding),self.V3(slotType_embedding)
            event2slot_score=torch.bmm(event_query,torch.transpose(slot_key,2,1))
            event2slot_score=torch.softmax((event2slot_score/128**0.5),dim=2)
